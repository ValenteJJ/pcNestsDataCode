...
Fitting a second version of the model in which true nest density is an explanatory variable for detection probability.
...
y2 = surveys %>% 
  ...
  mutate(count = ifelse(is.na(count), 0, count)) %>% 
  pivot_wider(names_from = distanceCat, values_from = count) %>% 
  full_join(pointData %>% select(year, point, date, nests, propOverlap2011, propOverlap2012_2014), by=c('year', 'point', 'date')) %>% 
  filter(!(year==2011 & propOverlap2011 < 0.8)) %>%
  filter(!(year > 2011 & propOverlap2012_2014 < 0.8)) %>% 
  arrange(year, point, date, time) %>% 
  select(-year, -point, -date, -time, -nests, -propOverlap2011, -propOverlap2012_2014, -geometry) %>% 
  as.matrix()
...
siteVars2 = siteVars %>% 
  ...
  mutate(year = as.factor(as.character(year))) %>% 
  arrange(year, point, date, time) %>% 
  select(plot, year, numPoint, scaleDoy, scaleTime, nests, numObserver)


analysisSpAbund2 = list('y' = y2, 'covs' = siteVars2, 'dist.breaks'=c(0, 20, 40, 60, 80, 100), 'offset'=pi)

modelSpAbund2 = DS(abund.formula= ~ plot + year + year*plot + (1|numPoint),
                 det.formula= ~ scaleDoy + scaleTime + I(scaleTime^2) + nests + (1|numObserver), data=analysisSpAbund2,
                 n.batch=6400, batch.length=25, accept.rate=0.43, family='Poisson', transect='point', det.func='halfnormal', n.burn=80000, n.thin=40, n.chains=3, inits=list(beta=coef(modelUnmarked)[1:48], alpha=c(4, coef(modelUnmarked)[69:71], 0)))
...
```{r}
ggplot(quality, aes(x=nestDensity, y=fledglingDensity))+
  geom_point()+
  geom_smooth()+
  theme_bw()+
  theme(panel.grid=element_blank())
...
jpeg('figureS1.jpg', width=3.5, height=3.5, units='in', res=300)
pointDens1Week
dev.off()
```
