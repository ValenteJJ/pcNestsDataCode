...
groundhog.library(lme4, '2025-04-01')
...
mean(pointRegResults$r2mLin)
quantile(pointRegResults$r2mLin, probs=c(0.025, 0.975))

mean(pointRegResults$r2cLin)
quantile(pointRegResults$r2cLin, probs=c(0.025, 0.975))
...
## Section 2.5 & 3.2 \| Point counts as indicators of population growth
...
#    slope2Quad[i] = coef(summary(model2))[3,1]
#    aicQuad[i] = AIC(model2)
#    r2mLin[i] = r.squaredGLMM(model2)[1]
#    r2cLin[i] = r.squaredGLMM(model2)[2]
#  }
# 
# 
...
mean(plotLamRegResults$r2mLin)
quantile(plotLamRegResults$r2mLin, probs=c(0.025, 0.975))
...
plotComparison =  predicted %>% 
  ...
  mutate(year = as.factor(year)) %>% 
  rename('pcEst' = 'est') %>% 
  mutate(year = recode(year, 'lambda2012'='Growth 2011-2012',
                       'lambda2013'='Growth 2012-2013',
                       'lambda2014'='Growth 2013-2014'))


#Create the plot
plotLevelLambda = ggplot()+
  geom_abline(intercept=0, slope=1, col='black', linetype='dashed')+
  ...
  theme(legend.position = c(0.25, 0.8),
        ...
        legend.background=element_blank())+
  guides(color='none')+
  ggtitle('Plot level growth rate')
...
```{r}
jpeg('figure3.jpg', width=3.5, height=3.5, units='in', res=300)
plotLevelLambda
dev.off()
```

## Section 2.6 & 3.3 \| Point counts as indicators of habitat quality
...
mean(plotFledgDensRegResults$r2mLin)
quantile(plotFledgDensRegResults$r2mLin, probs=c(0.025, 0.975))
...
densityQuality = ggplot()+
  ...
  xlab('Fledglings density (per hectare)')+
  ylab('Point count density estimates')+
  theme(legend.position='bottom',
        legend.box='vertical')+
  theme(legend.position = c(0.8, 0.8),
        ...
        legend.background=element_blank())+
  guides(color='none')+
  ggtitle('(A) Density vs. habitat quality')
...
mean(plotLamFledgDensResults$r2mLin)
quantile(plotLamFledgDensResults$r2mLin, probs=c(0.025, 0.975))
...
growthQuality = ggplot()+
  ...
  theme(panel.grid=element_blank())+
  geom_errorbar(data=plotComparison, aes(x=fledgDens, y=pcEst, color=plot, ymin=lcl, ymax=ucl))+
  xlab('Fledgling density (per hectare)')+
  ylab('Point count growth rate estimate')+
  theme(legend.position='bottom',
        legend.box='vertical')+
  theme(legend.position = c(0.7, 0.8),
        ...
        legend.background=element_blank())+
  guides(color='none')+
  ggtitle('(B) Growth rate vs. habitat quality')
...
jpeg('figure4.jpg', width=3.5, height=7, units='in', res=300)
plot_grid(densityQuality, growthQuality, ncol=1)
dev.off()
...
Modeling reproductive output (fledglings per nest) as a function of reproductive activity (nest density) to evaluate whether per-capita fitness decreases with competitor density.

```{r}
tmp = dailyNests %>%
  select(year, plot, meanNestDensity) %>% 
  full_join(quality, by=c('year', 'plot'))
  
model0 = lmer(fledgedPerNest ~ (1|year) + (1|plot), data=tmp)
model1 = lmer(fledgedPerNest ~ meanNestDensity + (1|year) + (1|plot), data=tmp)

anova(model0, model1)

summary(lmer(fledgedPerNest ~ meanNestDensity + (1|year) + (1|plot), data=tmp))

fledgPerNest = ggplot(tmp, aes(x=meanNestDensity, y=fledgedPerNest, color=plot))+
  geom_smooth(method='lm', color='black')+
  geom_point()+
  theme_bw()+
  theme(panel.grid=element_blank())+
  xlab('True nest density (nests per ha)')+
  ylab('Fledglings per nest')+
  theme(legend.position='none')
...
```{r}
jpeg('figure5.jpg', width=3.5, height=3.5, units='in', res=300)
fledgPerNest
dev.off()
```
...
mean(pointRegResults1Week$r2mLin)
quantile(pointRegResults1Week$r2mLin, probs=c(0.025, 0.975))
...
pointDens1Week = ggplot()+
  ...
  theme(panel.grid=element_blank())+
  # geom_errorbar(data=pointComparison, aes(x=nests, y=estimate, ymin=lcl, ymax=ucl))+
  xlab('True nest density (nests per ha) one week later')+
  ylab('Point count density estimates')+
  theme(legend.position='bottom',
        legend.box='vertical')
...
jpeg('figureS1.jpg', width=4, height=3.5, units='in', res=300)
pointDens1Week
dev.off()
```
...
Plotting the relationship between estimated survey-level density based on point counts and true nest density 2 weeks later.

```{r}
intercept=pointRegResults2Weeks$intercept
slope=pointRegResults2Weeks$slope

mean(pointRegResults2Weeks$r2mLin)
quantile(pointRegResults2Weeks$r2mLin, probs=c(0.025, 0.975))
...
pointDens2Weeks = ggplot()+
  ...
  theme(panel.grid=element_blank())+
  # geom_errorbar(data=pointComparison, aes(x=nests, y=estimate, ymin=lcl, ymax=ucl))+
  xlab('True nest density (nests per ha) two weeks later')+
  ylab('Point count density estimates')+
  theme(legend.position='bottom',
        legend.box='vertical')
...
```{r}
jpeg('figureS2.jpg', width=4, height=3.5, units='in', res=300)
pointDens2Weeks
dev.off()
```

## Figure S3 \| Point counts as indicators of habitat quality measured as per-capita reproduction

Here we are refitting the models from section 2.6 using per-capita reproduction as the metric of habitat quality instead of fledglings per hectare.
...
# 
# 
# plotFledgPerCapRegResults = data.frame(
#   'intercept'=intercept,
#   'slope'=slope,
#   'interceptQuad'=interceptQuad,
#   'slopeQuad'=slopeQuad,
...
# )
# 
# save(plotFledgPerCapRegResults, file='plotFledgPerCapRegResults.RData')
...
```{r}
load('plotFledgPerCapRegResults.RData')


plotFledgPerCapRegResults %>% 
  mutate(diffAic = aicQuad - aicLin) %>% 
  ggplot(aes(x=diffAic))+
  geom_histogram(fill='white', color='black')+
  geom_vline(xintercept=0, color='red')+
...
sum(plotFledgPerCapRegResults$aicLin < plotFledgPerCapRegResults$aicQuad)/nrow(plotFledgPerCapRegResults)
```

```{r}
intercept=plotFledgPerCapRegResults$intercept
slope=plotFledgPerCapRegResults$slope

mean(slope)
quantile(slope, probs=c(0.025, 0.975))

mean(plotFledgPerCapRegResults$r2mLin)
quantile(plotFledgPerCapRegResults$r2mLin, probs=c(0.025, 0.975))

simFledgPerCap = seq(0, 3, 0.1)
ests = rep(NA, length(simFledgPerCap))
lcl = rep(NA, length(simFledgPerCap))
ucl = rep(NA, length(simFledgPerCap))

for(i in 1:length(simFledgPerCap)){
  tmp = intercept + slope*simFledgPerCap[i]
  ests[i] = mean(tmp)
  lcl[i] = quantile(tmp, probs=0.025)
  ucl[i] = quantile(tmp, probs=0.975)
}
...
plotComparison = predicted %>% 
  ...
  summarise(estimate = mean(pcEst),
         lcl = quantile(pcEst, probs=0.025),
         ucl = quantile(pcEst, probs=0.975),
         fledgPerCap = max(fledgedPerNest)) %>% 
  ungroup() %>% 
  mutate(year = as.factor(as.character(year))) %>% 
  rename('pcEst' = 'estimate')

#Create the plot
densityQualityPerCap = ggplot()+
  geom_ribbon(data=regPreds, aes(x=fledgPerCap, ymin=lclPC, ymax=uclPC), alpha=0.2)+
  geom_line(data=regPreds, aes(x=fledgPerCap, y=predPC), color='black')+
  geom_point(data=plotComparison, aes(x=fledgPerCap, y=pcEst, color=plot, pch=year))+
  theme_bw()+
  theme(panel.grid=element_blank())+
  geom_errorbar(data=plotComparison, aes(x=fledgPerCap, y=pcEst, color=plot, ymin=lcl, ymax=ucl))+
...
densityQualityPerCap
```

```{r}

jpeg('figureS3.jpg', width=4, height=3.5, units='in', res=300)
densityQualityPerCap
dev.off()
```
...
Calculating plot-level fledglings per nest.

```{r}

qualityLambda = quality %>% 
  select(plot, year, fledgedPerNest) %>% 
  mutate(year = ifelse(year==2011, 'lambda2012', year)) %>% 
  mutate(year = ifelse(year==2012, 'lambda2013', year)) %>% 
  mutate(year = ifelse(year==2013, 'lambda2014', year)) %>% 
  filter(year != 2014)
...
Modeling the posterior distribution of estimated point count lambda as a linear and quadratic function of fledglings per nest. For each iteration, we record the intercept and slope estimates along with the AIC values.
...
#      filter(iteration==i)
# 
#    model = lm(pcEst ~ fledgedPerNest, data=tmp)
#    intercept[i] = coef(summary(model))[1,1]
#    slope[i] = coef(summary(model))[2,1]
#    aicLin[i] = AIC(model)
#    r2mLin[i] = r.squaredGLMM(model)[1]
#    r2cLin[i] = r.squaredGLMM(model)[2]
# 
#    model2 = lm(pcEst ~ fledgedPerNest + I(fledgedPerNest^2), data=tmp)
#    interceptQuad[i] = coef(summary(model2))[1,1]
#    slopeQuad[i] = coef(summary(model2))[2,1]
#    slope2Quad[i] = coef(summary(model2))[3,1]
#    aicQuad[i] = AIC(model2)
...
# 
# 
# plotLamFledgPerCapResults = data.frame(
#   'intercept'=intercept,
#   'slope'=slope,
#   'interceptQuad'=interceptQuad,
#   'slopeQuad'=slopeQuad,
...
# )
# 
# save(plotLamFledgPerCapResults, file='plotLamFledgPerCapResults.RData')
```
...
load('plotLamFledgPerCapResults.RData')

plotLamFledgPerCapResults %>% 
  mutate(diffAic = aicQuad - aicLin) %>% 
  ggplot(aes(x=diffAic))+
  geom_histogram(fill='white', color='black')+
  geom_vline(xintercept=0, color='red')+
...
sum(plotLamFledgPerCapResults$aicLin < plotLamFledgPerCapResults$aicQuad)/nrow(plotLamFledgPerCapResults)
```

Plotting the relationship between estimated plot-level lambdas based on point counts and fledglings per nest.

```{r}

intercept=plotLamFledgPerCapResults$intercept
slope=plotLamFledgPerCapResults$slope

mean(slope)
quantile(slope, probs=c(0.025, 0.975))

mean(plotLamFledgPerCapResults$r2mLin)
quantile(plotLamFledgPerCapResults$r2mLin, probs=c(0.025, 0.975))
...
#Create the plot
growthQualityPerCap = ggplot()+
  geom_ribbon(data=regPreds, aes(x=fledgedPerNest, ymin=lclPC, ymax=uclPC), alpha=0.2)+
  geom_line(data=regPreds, aes(x=fledgedPerNest, y=predPC), color='black')+
  geom_point(data=plotComparison, aes(x=fledgedPerNest, y=pcEst, color=plot, pch=year))+
  theme_bw()+
  theme(panel.grid=element_blank())+
  geom_errorbar(data=plotComparison, aes(x=fledgedPerNest, y=pcEst, color=plot, ymin=lcl, ymax=ucl))+
  xlab('Fledglings per nest')+
  ylab('Point count growth rate estimate')+
  theme(legend.position='bottom',
        legend.box='vertical')+
...
```{r}
jpeg('figureS4.jpg', width=4, height=3.5, units='in', res=300)
growthQualityPerCap
dev.off()
```

## Figure S5 \| Effects of nest density on detectability of Wood Thrush
...
siteVars2 = surveys %>% 
  ...
  mutate(scaleDoy = scale(doy, center=T, scale=T),
         ...
         numObserver = as.integer(as.factor(observer)),
         numPoint = as.integer(as.factor(as.character(point)))) %>% 
  left_join(pointData %>% select(point, year, date, nests), by=c('point', 'year', 'date')) %>% 
  mutate(year = as.factor(year))


analysisSpAbund2 = list('y' = y2, 'covs' = siteVars2, 'dist.breaks'=c(0, 20, 40, 60, 80, 100), 'offset'=pi)

# modelSpAbund2 = DS(abund.formula= ~ plot + year + year*plot + (1|numPoint), det.formula= ~ scaleDoy + scaleTime + I(scaleTime^2) + nests + (1|numObserver), data=analysisSpAbund2, n.batch=6400, batch.length=25, accept.rate=0.43, family='Poisson', transect='point', det.func='halfnormal', n.burn=80000, n.thin=40, n.chains=3, inits=list(beta=coef(modelUnmarked)[1:48], alpha=c(4, coef(modelUnmarked)[69:71], 0)))
# 
# save(modelSpAbund2, file='modelSpAbund2.RData')
```

```{r}

# load('modelSpAbund2.RData')


# tmp1 = modelSpAbund$N.samples
# tmp2 = modelSpAbund2$N.samples
# tmpData = tmp1 - tmp2
# 
# 
# 
# #Record results from linear model pcDensity ~ nestDensity
# intercept = rep(NA, nrow(tmpData))
# slope = rep(NA, nrow(tmpData))
# 
# for(i in 1:length(intercept)){
# 
#   print(i)
# 
#   tmp = data.frame('diffAbund' = tmpData[i,],
#                  'point' = siteVars2$point,
#                  'year' = as.numeric(as.character(siteVars2$year)),
#                  'date' = siteVars2$date) %>% 
#   full_join(pointData, by=c('point', 'year', 'date')) %>% 
#   mutate(diffDens = diffAbund / pi,
#          nestsHa = nests / pi)
# 
#   model = lme4::lmer(diffDens ~ nestsHa + (1|point) + (1|year), data=tmp)
#   intercept[i] = coef(summary(model))[1,1]
#   slope[i] = coef(summary(model))[2,1]
#   }
#   
...
# 
# 
# save(diffDens, file='diffDens.RData')
...
#Summarize the values estimated for the original data points similarly
pointComparison =  data.frame(pointData) %>% 
  mutate(estimate = colMeans(tmpData / pi, na.rm=T),
         lcl = apply(tmpData / pi, 2, quantile, probs=0.025, na.rm=T),
         ucl = apply(tmpData / pi, 2, quantile, probs=0.975, na.rm=T)) %>% 
  # filter(!(year==2011 & propOverlap2011 < 0.8)) %>%
  # filter(!(year > 2011 & propOverlap2012_2014 < 0.8)) %>% 
  mutate(nestDens = nests/((pi*100^2)/10000))
  #        estimate = estimate/((pi*100^2)/10000),
...
diffDens = ggplot()+
  ...
  # geom_errorbar(data=pointComparison, aes(x=nests, y=estimate, ymin=lcl, ymax=ucl))+
  xlab('True nest density (nests per ha)')+
  ylab('Difference in density estimates')

diffDens
```

```{r}
jpeg('figureS5.jpg', width=4, height=3.5, units='in', res=300)
diffDens
dev.off()
```

## Table S1 \| Data collected by plot and year
...
# detections
# detectionsPerSurvey
# nestsActiveDuringPCs
# successfulNests
# fledglings
# fledglingsPerNest
# fledglingsPerSuccessfulNest


tmp = data.frame(nestData) %>% 
  group_by(nestYear, plot) %>% 
  summarise(nestsActive = n(),
            successfulNests = sum(fledged > 0),
            fledglings = sum(fledged)) %>% 
  mutate(fledglingsPerNest = fledglings/nestsActive,
         fledglingsPerSuccessfulNest = fledglings/successfulNests) %>% 
  rename(year = nestYear) %>% 
  left_join(data.frame(plots) %>% 
              select(plot, year, hectares), by=c('plot', 'year'))
...
tmp1 = birds %>% 
  ...
  group_by(year, plot) %>% 
  summarise(pcDetections = n()) %>% 
  ungroup() %>% 
  full_join(tmp, by=c('plot', 'year'))
  

tmp2 = surveys %>% 
  select(year, point) %>% 
  left_join(pcLocations %>% 
              select(point, plot), by='point') %>% 
  unique() %>% 
  group_by(year, plot) %>% 
  summarise(pcPoints = n()) %>% 
  full_join(tmp1, by=c('year', 'plot'))
...
tmp3 = surveys %>% 
  ...
  group_by(year, plot) %>% 
  summarise(surveys = n()) %>% 
  full_join(tmp2, by=c('year', 'plot')) %>% 
  mutate(detsPerSurvey = pcDetections / surveys)

tmp4 = tmp3 %>% 
  select(year, plot, hectares, pcPoints, surveys, pcDetections, detsPerSurvey, nestsActive, successfulNests, fledglings, fledglingsPerNest, fledglingsPerSuccessfulNest)

write.csv(tmp4, 'tableS1.csv')
